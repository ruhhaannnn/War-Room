...
// --- INSTANT CACHE ENGINE (WITH AUTO-REPAIR) ---
...
// --- FAST PROXY ENGINE (Added Corsproxy.io Fallback for Rate Limits) ---
async function fetchWithFastestProxy(targetUrl, type = 'json') {
    ...
    throw new Error("All Proxies failed due to rate limits.");
}
...
async function fetchSummary() {
    try {
        ...
        let recent = [...(presstv||[]), ...(meObs||[]), ...(rnintel||[])]
            .filter(p => (Date.now() - p.date.getTime()) < 3600000 && p.text.length <= 160) // Short text only
...
// AUTO-REPAIRING LOCALSTORAGE DB (Handles missing timestamps safely)
function getStoredIntel() { 
    try {
        let data = JSON.parse(localStorage.getItem('iqwr_intel_db')) || [];
        // Strict filter to remove corrupted objects from previous code versions
        let cleanedData = data.filter(d => typeof d.timestamp === 'number' && !isNaN(d.timestamp));
        return cleanedData;
    } catch(e) {
        // If unreadable, wipe it and start fresh to stop site crashing
